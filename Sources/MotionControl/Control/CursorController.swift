...
class CursorController {
    ...
    /// 权重混合模型：光标平滑位置（一阶Lerp，无速度/惯性）
    private var phy_x: CGFloat = 0
    private var phy_y: CGFloat = 0
    ...
    func updateWithAbsolutePosition(
        ...
        // 权重混合模型：手位置 + 最近按钮中心 → 加权融合 → 一阶Lerp逼近
        let config = ConfigManager.shared.currentConfig
        if config.assistEnabled {
            let rawTarget = handTarget

            if phy_x == 0 && phy_y == 0 { phy_x = currentPosition.x; phy_y = currentPosition.y }

            // 扫最近按钮
            var minButtonDist = CGFloat.greatestFiniteMagnitude
            var nearestCenter = rawTarget  // 默认=手位置，无按钮时不出界
            if let scanner = uiScanner, !scanner.cachedElements.isEmpty {
                for el in scanner.cachedElements {
                    let cx = el.frame.midX; let cy = screenSize.height - el.frame.midY
                    let d = sqrt(pow(cx-phy_x,2) + pow(cy-phy_y,2))
                    if d < minButtonDist { minButtonDist = d; nearestCenter = CGPoint(x: cx, y: cy) }
                }
            }
            ...
            // 混合目标：手 × (1-w²) + 按钮 × w²，钳在屏幕内
            let blendTarget = CGPoint(
                x: max(0, min(rawTarget.x * (1 - w2) + nearestCenter.x * w2, screenSize.width)),
