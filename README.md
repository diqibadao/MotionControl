...
- 👀 **纯手指光标控制** — 指尖 Delta 灵敏度 + 加速曲线，Velcity EMA 平滑
- 💠 **60fps 补帧** — 30fps 检测 + 60fps 输出，光标丝滑
- 👄 **语音输入** — 张嘴即说，闭嘴即输
...
| 鼠标光标移动 | Core Graphics `CGWarpMouseCursorPosition` | ✅ 系统原生 | 真·系统级光标移动 |
...
- [x] **指尖 Delta 灵敏度 + 加速曲线**：速度基加速度（慢→0.4x，中→1x，快→1x~4x），非对称灵敏度（右手右/下 1.5x），Velocity EMA 平滑（α=0.3），方向一致性检查防漂移，固定 dt 线性化
- [x] **60fps 补帧输出**：30fps 检测 + 60fps Timer 插值，光标更丝滑
- [x] **纯手指光标模式**：注视偏移已禁用，保留面部检测和 overlay
- [x] 手势 → 鼠标动作映射（CGEventSource抑制归零 + CGAssociate + CGWarp + CGEvent 四管齐下）
- [x] **手势映射可配置**：每个手势可绑定不同动作（下拉选择）
- [x] **磁性吸附**：光标自动吸附到按钮/输入框等 UI 元素，支持 Dock 底部图标
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v0.4 | 2026-05-31 | 加速曲线 + Velocity EMA + 60fps补帧 + 激活阈值收紧 + 非对称灵敏度 + 方向一致性 + 固定dt |
| v0.3 | 2026-05-30 | 位置 EMA 替换 velocity 累积 + 磁吸 Dock + 面部姿态修复 + 纯手指模式 |
